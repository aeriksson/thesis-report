...
As mentioned in the section~\ref{sect:problem_formulation}, the problem space must be limited in order for it to admit an oracle. The goal of this section is to present a problem set that is general enough to admit most of the problems while remaining limited enough to be useful in practice.
...
The components can then be defined as (where $S(\mathbf{x})$ is the set of indexed subsequences of some sequence \mathbf{x}, e.g. $S((x_1, x_2)) = \{((x_1), (1)), ((x_2), (2)), ((x_1, x_2), (1, 2))\}$):
\begin{description}
...
\begin{algorithmic}
    \Require{A sequence $\mathbf{x}$ and a tuple $(F_E, C, F_T, M, A)$.}
    ...
    \For{$(\mathbf{x}_i, \mathbf{i}_i) \in F_E(\mathbf{x})$} \Comment{iterate over subsequences (and indexes) selected by filter}
        \State{$\{\mathbf{c}_1, \mathbf{c}_2, \dots, \mathbf{c}_j\} \gets C(\mathbf{i}_i)$ \Comment{acquire context}}
        ...
        \State{$S \gets S \cup (M(\mathbf{x}_i, \{\mathbf{t}_1, \mathbf{t}_2, \dots, \mathbf{t}_k\}), \mathbf{i}_i)$ \Comment{save anomaly score with indexes of subsequence}}
    \EndFor{}
...
\begin{algorithmic}
    ...
    \For{$\mathbf{x}_i \in S(\mathcal{X})$} \Comment{iterate over sequences}
        \State{$a_i \gets M(\mathbf{x}_i, C(\mathcal{X}, i))$ \Comment{save anomaly score}}
    \EndFor{}
